 ...
 * height = 200
 * nrow = ncol = 3
 * row_space = height / nrow
 * col_space = width / ncol
 * buttonRadius = min(row_space, col_space)
 *
 * buttonColor = white;
 * buttonHitColor=Red
 * segments = []
 ...
 * for i = 0 to nrow
 * 	for j = 1 to nrow
 * 		buttonCenter.x = i * col_space + col_space / 2
 * 		buttonCenter.y = j * row_space + row_space / 2
 * 	fi
 * fi
 *
 *
 ...
 * 	path=path+point
 *	findHits(points)
 * 	generateScreen(points, hits)
 * else event==up
 * 	send(findHits(points))
 * 	inTouch = false
 * 	points=[]
 * fi
 *
 ...
 * findHits( points )
 * 	for each segment
 * 	if segment intersects button
 * 	hits = hits + button
 *
 * function intersect( segment, buttons)
 * 	rv = -1
 * 	bc=-1
 * 	while rv < 0
 * 		bc++;
...
public class LockPatternView extends View {
    ...
    #DEFINE ASPECT_SQUARE  0; // View will be the minimum of width/height
    #DEFINE ASPECT_LOCK_WIDTH  1; // Fixed width; height will be minimum of (w,h)
    #DEFINE ASPECT_LOCK_HEIGHT  2; // Fixed height; width will be minimum of (w,h)
     ...
     */
    #DEFINE MILLIS_PER_CIRCLE_ANIMATING  700;
